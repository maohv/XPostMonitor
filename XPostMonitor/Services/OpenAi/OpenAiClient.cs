        ...
        string? chainImageStyle, CancellationToken cancellationToken)
    {
        ...
        var requestBody = new
        {
            ...
            instructions = "Act as a fast, sharp meme-token trader and editor. Choose the one hook that would make a reader stop, "
                ...
                + "First decide whether the reply or quote confirms, mocks, rejects, explains, or extends the original Post. If the "
                + "original Post has the strongest surprising word, unusual capitalization, joke, nickname, or punchline and the "
                + "new Post only reacts to or explains it, keep the original hook. If the new Post introduces a stronger hook, use "
                + "that new hook. If the memorable idea exists only through their relationship, name that combined story. Never "
                + "default to a generic summary of the latest sentence. For example, when an original Post proposes moving users "
